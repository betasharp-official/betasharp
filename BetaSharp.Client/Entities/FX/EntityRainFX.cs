...
public class EntityRainFX : EntityFX
{
    ...
    public override void tick()
    {
        ...
        Material material = _ctx.getMaterial(MathHelper.Floor(x), MathHelper.Floor(y), MathHelper.Floor(z));
        if (material.IsFluid || material.IsSolid)
        {
            double height = (double)((float)(MathHelper.Floor(y) + 1) - BlockFluid.getFluidHeightFromMeta(_ctx.getBlockMeta(MathHelper.Floor(x), MathHelper.Floor(y), MathHelper.Floor(z))));
            if (y < height)
            {
                markDead();
            }
