...
public class BlockRenderer
{
    ...
    public static bool RenderBlockByRenderType(IBlockReader world, ILightProvider lighting, Block block, BlockPos pos,TextureManager texManager, Tessellator tess, int overrideTexture = -1, bool renderAllFaces = false)
    {
    ...
    public static void RenderBlockOnInventory(Block block, int metadata, float brightness, Tessellator tess, TextureManager texManager)
    {
        ...
        else
        {
            ...
            tess.startDrawingQuads();
            tess.setNormal(0.0F, 1.0F, 0.0F);
            RenderBlockByRenderType(itemWorld, itemWorld, block, itemPos, texManager, tess, uiCtx.OverrideTexture, true);
            tess.draw();
            GLManager.GL.Translate(0.5F, 0.5F, 0.5F);
        }
    }

    public static void RenderBlockFallingSand(Block block, IWorldContext world, int x, int y, int z, Tessellator tess, TextureManager texManager)
    {
