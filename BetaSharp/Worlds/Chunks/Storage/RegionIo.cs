...
internal class RegionIo
{
    ...
    public static RegionFile func_22193_a(string var0, int var1, int var2)
    {
        lock (l)
        {
            ...
            if (cache.TryGetValue(var4, out WeakReference<RegionFile>? weakRef) && weakRef.TryGetTarget(out RegionFile? var6) && var6 != null)
            {
                return var6;
            }
    ...
    public static void flush()
    {
        lock (l)
        {
            ...
            {
                if (pair.Value.TryGetTarget(out RegionFile? regionFile) && regionFile != null)
                {
                    try
                    {
