        ...
        while (true)
        {
            ...
            if ((blockId != 0 && blockId != Block.Leaves.id) || y <= 0)
            {
                for (int i = 0; i < 128; ++i)
                {
                    ...
                    if (level.BlocksReader.IsAir(genX, genY, genZ) && ((BlockPlant)Block.Blocks[_tallGrassBlockId]).canGrow(new OnTickEvt(level, genX, genY, genZ, level.BlocksReader.GetBlockMeta(genX, genY, genZ), level.BlocksReader.GetBlockId(genX, genY, genZ))))
                    {
