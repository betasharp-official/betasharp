...
internal class SugarCanePatchFeature : Feature
{
    public override bool Generate(World world, JavaRandom rand, int x, int y, int z)
    {
        for (int i = 0; i < 20; ++i)
        {
            ...
            bool hasWaterNearby = world.BlocksReader.getMaterial(genX - 1, y - 1, genZ) == Material.Water ||
                                  world.BlocksReader.getMaterial(genX + 1, y - 1, genZ) == Material.Water ||
                                  world.BlocksReader.getMaterial(genX, y - 1, genZ - 1) == Material.Water ||
                                  world.BlocksReader.getMaterial(genX, y - 1, genZ + 1) == Material.Water;
